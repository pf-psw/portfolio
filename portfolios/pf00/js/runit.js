...
const IS_HIGH_END_DEVICE = function() {
...
const MAX_WIDTH = $(document).width();
const MAX_HEIGHT = $(document).height();
...
const isRunning = function(state=store.state) {return true};
// Whether any sounds are allowed, taking into account multiple factors.
const canPlaySoundSelector = function(state=store.state) { return isRunning(state) && soundEnabledSelector(state)};
// Convert quality to number.
const qualitySelector = function() { return +store.state.config.quality};
const shellNameSelector = function() { return store.state.config.shell};
// Convert shell size to number.
const shellSizeSelector = function() { return +store.state.config.size};
const finaleSelector = function() { return store.state.config.finale};
const skyLightingSelector = function() { return +store.state.config.skyLighting};
const scaleFactorSelector = function() { return store.state.config.scaleFactor};
...
// Unique shell types
const crysanthemumShell = function(size=1){
...
const ghostShell = function(size=1) {
...
const strobeShell = function(size=1) {
...
const palmShell = function(size=1) {
...
const ringShell = function(size=1) {
	const color = randomColor();
...
const crossetteShell = function(size=1) {
	const color = randomColor({ limitWhite: true });
	return {
		shellSize: size,
		spreadSize: 300 + size * 100,
...
const floralShell = function(size=1) {return{
	shellSize: size,
...
const fallingLeavesShell = function(size=1) {return{
	shellSize: size,
	color: INVISIBLE,
	spreadSize: 300 + size * 120,
	starDensity: 0.12,
...
const willowShell = function(size=1) {return{
	shellSize: size,
	spreadSize: 300 + size * 100,
	starDensity: 0.6,
...
const crackleShell = function(size=1) {
	// favor gold
...
const horsetailShell = function(size=1) {
	const color = randomColor();
	return {
		shellSize: size,
		horsetail: true,
...
function handleResize() {
	...
	stages.forEach(function(stage) {stage.resize(containerW, containerH)});
	// Account for scale
...
const currentSkyColor = { r: 255, g: 0, b: 0 };
const targetSkyColor = { r: 255, g: 0, b: 0 };
...
class Shell {
	...
	burst(x, y) {
		...
		if (this.crossette) onDeath = function(star) {
			if (!playedDeathSound) {
				playedDeathSound = true;
			}
			crossetteEffect(star);
		}
		if (this.crackle) onDeath = function(star) {
			if (!playedDeathSound) {
				playedDeathSound = true;
			}
			crackleEffect(star);
